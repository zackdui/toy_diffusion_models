...
torch.save(model.state_dict(), "unet_model_mnist_labels.pth")
...
with torch.no_grad():
    ...
    traj = torchdiffeq.odeint(
            lambda t, x: model.forward(t, x, torch.tensor(list(range(10)), device=device)),
...
os.makedirs("outputs_mnist_cfm_labels", exist_ok=True)
for i, img in enumerate(images):
    save_path = f"outputs_mnist_cfm_labels/sample_{i}.png"
